...
describe('Task', () => {
  describe('mapFromRawTask', () => {
    it('raw string maps to correct task', () => {
      ...
      expect(mappedtask.supportRequirements).toEqual(['Picking up groceries/medications']);
    });
  });
  describe('canBeFulfilledByVolunteer', () => {
    ...
    it('should return false if volunteer does not have any supporting capabilities', () => {
      ...
      when(getMock).calledWith('I can provide the following support (non-binding)').mockReturnValue([]);
    ...
    it('should return false if volunteer does have capabilities but none supporting the task at hand', () => {
      const getMock = jest.fn();
      when(getMock).calledWith('I can provide the following support (non-binding)').mockReturnValue(['Check-in on folks throughout the day (in-person or phone call)']);
      const volunteer = { get: getMock };
      const task = Task.mapFromRawTask('Grocery shopping');
      expect(task.canBeFulfilledByVolunteer(volunteer)).toBeFalsy();
    });
    it('should return true if volunteer does have supporting capabilities', () => {
      const getMock = jest.fn();
      when(getMock).calledWith('I can provide the following support (non-binding)').mockReturnValue(['Picking up groceries/medications']);
      const volunteer = { get: getMock };
      const task = Task.mapFromRawTask('Grocery shopping');
    ...
    it('should return false if task requires volunteer to have a car, but volunteer does not have one', () => {
      ...
      when(getMock)
        ...
        .mockReturnValue([])
        .calledWith('Do you have a private mode of transportation with valid license/insurance? ')
        .mockReturnValue([]);
      const volunteer = { get: getMock };
      const task = Task.mapFromRawTask('Transportation to/from a medical appointment');
      expect(task.canBeFulfilledByVolunteer(volunteer)).toBeFalsy();
    });
